...
def main():
    ...
    # Rule-based flood risk (demo)
    def calculate_flood_risk(row):
        elev = row['Elevation'] if row['Elevation'] > 0 else 1.0
        hist_score = row['Historical_Flood_Score'] if not pd.isna(row['Historical_Flood_Score']) else 0.0
        score = (0.4 * row['Rain_mm']/50.0) + \
                (0.2 * row['Rain_Past3h']/150.0) + \
                (0.15 * (1.0/elev)) + \
                (0.15 * (row['Drain_Water_Level']/2.0)) + \
                (0.1 * hist_score)
        if score > 0.7:
            return "High"
        elif score > 0.4:
    ...
    print("[Step] Calculating flood risk (demo rule-based)...")
    grid_hours['Flood_Risk'] = grid_hours.apply(calculate_flood_risk, axis=1)
